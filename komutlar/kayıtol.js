...
module.exports.run = async (client, msg, args) => {
 ...
 if (!member) return msg.channel.send(":x: Bir Üye Etiketlemelisin!");
  if (!isim) return msg.channel.send(":x: Bir İsim Yazmalısın!");
  const db = require('quick.db')
  let kullanıcı = await db.fetch(`ksistem_${msg.guild.id}`);
   member.setNickname(`${isim}`);
  if( kullanıcı == undefined){
msg.reply("**Kayıt komutları kapalı açmak için g!kayıt-sistemi aç**")
  }else{
      if( kullanıcı == "acik"){
  ...
  if (msg.channel.id !== kanal) return msg.channel.send(`Bu komutu sadece <#${kanal}> kanalında kullanabilirsin.`).then(msg => msg.delete(10000))
    ...
    msg.channel.send({
        embed: {
            color: Math.floor(Math.random() * (0x00000 + 1)),
            description: ("|Tebrikler kayıt oldunuz| Ayarlanan Rol Verildi !!.")
        }
