...
exports.run = async (client, message, args) => {
  	let sunucuyaözelayarlarOtorol = JSON.parse(fs.readFileSync("./otorol.json", "utf8"));
		if(!sunucuyaözelayarlarOtorol[message.guild.id]) {
			const embed = new Discord.RichEmbed()
				.setDescription(`** :tada: Otorolü Ayarlamadığın İçin Kapatamazsın!**`)
				.setColor("RED")
  ...
  exports.run = async (client, message, args, member) => {
		...
		const embed = new Discord.RichEmbed()
			.setDescription(`** :tada: Otorol Mesajları Başarıyla Kapatıldı**`)
			.setColor("RANDOM")
			.setTimestamp()
		message.channel.send({embed})
		return
...
exports.conf = {
  enabled: true, 
  guildOnly: false, 
  aliases: ["otorolmesajkapat","otomesajkapat "],
  permLevel: 0
};
